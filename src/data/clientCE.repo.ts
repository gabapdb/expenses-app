...
import {
  clientSchema,
  type ClientProject,
} from "@/domain/validation/clientSchema";
...
export async function getProjectCostEstimates(
  ...
  const project = parsed.data.projects?.[projectId] as ClientProject | undefined;

  if (!project) return {};
  if (!project.designPhase) return {};
  if (!project.designPhase.costEstimates) return {};
